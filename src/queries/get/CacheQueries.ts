...
export async function fetchReadOnlyTerms(
  ...
  for (let i = 0; i < Math.ceil(terms.length / sliceSize); i++) {
    ...
    const query = [
      ...
      "SELECT DISTINCT ?term ?termLabel ?termAltLabel ?termType ?termDefinition ?termDescription ?termSource ?topConcept ?inverseOnProperty ?restriction ?restrictionPred ?onProperty ?onClass ?target ?subClassOf ?scheme",
      "WHERE {",
      "GRAPH ?graph {",
