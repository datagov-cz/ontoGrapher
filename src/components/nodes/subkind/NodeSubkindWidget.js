...
export interface NodeSubkindWidgetProps {
    node: NodeSubkindModel;
    size?: number;
}

export interface NodeSubkindWidgetState {}

export class NodeSubkindWidget extends React.Component<NodeSubkindWidgetProps, NodeSubkindWidgetState>{
    constructor(props: NodeSubkindWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"subkind-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«subkind»</text>
...
NodeSubkindWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodeSubkindWidgetFactory = React.createFactory(NodeSubkindWidget);