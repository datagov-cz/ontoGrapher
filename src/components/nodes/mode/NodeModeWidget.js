...
export interface NodeModeWidgetProps {
    node: NodeModeModel;
    size?: number;
}

export interface NodeModeWidgetState {}

export class NodeModeWidget extends React.Component<NodeModeWidgetProps, NodeModeWidgetState>{
    constructor(props: NodeModeWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"mode-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«mode»</text>
...
NodeModeWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodeModeWidgetFactory = React.createFactory(NodeModeWidget);