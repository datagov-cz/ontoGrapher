...
export interface NodeRoleWidgetProps {
    node: NodeRoleModel;
    size?: number;
}

export interface NodeRoleWidgetState {}

export class NodeRoleWidget extends React.Component<NodeRoleWidgetProps, NodeRoleWidgetState>{
    constructor(props: NodeRoleWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"role-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«role»</text>
...
NodeRoleWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodeRoleWidgetFactory = React.createFactory(NodeRoleWidget);