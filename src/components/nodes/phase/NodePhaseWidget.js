...
export interface NodePhaseWidgetProps {
    node: NodePhaseModel;
    size?: number;
}

export interface NodePhaseWidgetState {}

export class NodePhaseWidget extends React.Component<NodePhaseWidgetProps, NodePhaseWidgetState>{
    constructor(props: NodePhaseWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"phase-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«phase»</text>
...
NodePhaseWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodePhaseWidgetFactory = React.createFactory(NodePhaseWidget);