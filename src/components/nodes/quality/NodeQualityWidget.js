...
export interface NodeQualityWidgetProps {
    node: NodeQualityModel;
    size?: number;
}

export interface NodeQualityWidgetState {}

export class NodeQualityWidget extends React.Component<NodeQualityWidgetProps, NodeQualityWidgetState>{
    constructor(props: NodeQualityWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"quality-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«quality»</text>
...
NodeQualityWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodeQualityWidgetFactory = React.createFactory(NodeQualityWidget);