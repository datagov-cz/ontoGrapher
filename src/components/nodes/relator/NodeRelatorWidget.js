...
export interface NodeRelatorWidgetProps {
    node: NodeRelatorModel;
    size?: number;
}

export interface NodeRelatorWidgetState {}

export class NodeRelatorWidget extends React.Component<NodeRelatorWidgetProps, NodeRelatorWidgetState>{
    constructor(props: NodeRelatorWidgetProps){
        super(props);
        this.state = {};
    }
    ...
    render(){
        ...
        return (
            <div className={"relator-node"} width={this.props.size} height={height} onDoubleClick={event => {let str = prompt("Enter name:"); this.changeName(str);this.forceUpdate();}}>
                <svg
                    ...
                    <g>
                        <rect fill="#ffffff" stroke={select} strokeWidth="3" width={this.props.size}
                              height={height}></rect>
                        <text width={this.props.size} textAnchor="middle" dominantBaseline="hanging" x="50%" y="5px" fill="#000000">«relator»</text>
...
NodeRelatorWidget.defaultProps = {
    size: 150,
    node: null
};

export var NodeRelatorWidgetFactory = React.createFactory(NodeRelatorWidget);