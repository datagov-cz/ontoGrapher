...
export class MaterialLinkModel extends DefaultLinkModel {
    constructor(){
        super("link-material");
        this.width = 3;
        this.curvyness = 0;
        let newLabel = new DefaultLabelModel();
        newLabel.setLabel("«material»");
...
export interface MaterialLinkProps extends BaseWidgetProps {
    color?: string;
    width?: number;
    smooth?: boolean;
...
export interface MaterialLinkState {
    selected: boolean;
}


export class MaterialLinkWidget extends DefaultLinkWidget {
    render() {
...
export class MaterialLinkFactory extends DefaultLinkFactory{
    constructor(){
        super();
        this.type = "link-material";
    }

    getNewInstance(initialConfig?: any): MaterialLinkModel{
        return new MaterialLinkModel();
    }

    generateReactWidget(diagramEngine: DiagramEngine, link: MaterialLinkModel): JSX.Element {
    ...
    generateLinkSegment(model: MaterialLinkModel, widget: MaterialLinkWidget, selected: boolean, path:string){
        var markerId= Toolkit.UID();
        var markerEndUrl = "url(#"+markerId+")";
        return (

                <path className={selected ? "link-material--path-selected" : "link-material"}
                      ref={ref => {
                          this.path = ref;
                      }}
...
export class MaterialPortModel extends DefaultPortModel{
    createLinkModel(): MaterialLinkModel | null {
        return new MaterialLinkModel();
    }
}